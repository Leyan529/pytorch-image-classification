...
def get_annotation(data_root):
    ...
    VOCdevkit_path  = os.path.join(data_root, "EdgeAOI")
    
    total_df = pd.read_csv(os.path.join(VOCdevkit_path, 'train.csv'))

    total_df['cls_path'] = total_df.apply(lambda x : cls_path(VOCdevkit_path, x['ID'],x['Label']),axis=1)
    total_df['path'] = total_df.apply(lambda x : img_path(VOCdevkit_path, x['ID']),axis=1)
    classes = list(total_df["Label"].unique())

    for imgSet in sets:
        for cls in classes:        
            if not os.path.exists(os.path.join(VOCdevkit_path, "Classification", imgSet, str(cls))):
                os.makedirs(os.path.join(VOCdevkit_path, "Classification", imgSet, str(cls)))
