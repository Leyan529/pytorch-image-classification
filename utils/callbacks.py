...
class LossHistory():
    def __init__(self, log_dir, model, input_shape, patience = 5):
        import datetime
    ...
    def write_summary(self, cpu_model, input_shape):
        print("write model summary ready")
        rndm_input = torch.autograd.Variable(torch.rand(1, 3, input_shape[0], input_shape[1]), requires_grad = False).cpu()
        ...
        with redirect_stdout(f):
            summary(cpu_model, (3, input_shape[0], input_shape[1]), device="cpu")
        lines = f.getvalue()
